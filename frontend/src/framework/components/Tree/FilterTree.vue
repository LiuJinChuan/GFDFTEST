  ...
  <div class="Tree">
    ...
    >
      <span class="custom-tree-node" slot-scope="{ node, data }">
        <i v-if="data.depts" class="el-icon-s-shop" style="margin-right:5px"></i>
        <i v-if="data.users" class="el-icon-user-solid" style="margin-right:5px"></i>
...
export default {
  ...
  methods: {
      ...
      if (!this.TreeData.list) {
        this.TreeData.ajax.Api.load().then((res) => {
          if (GFActor.user.datascope == 0) {
            ...
            all[0][this.TreeData.defaultProps.label] = '全部';
            all[0][this.TreeData.key] = '';
            all[0][this.TreeData.defaultProps.children] = res.data;

            /*是否开启显示人员*/
            if (this.TreeData.filter.ischildren) {
              this.TreeData.ajax.Api1.load().then((ress) => {
                ress.data.forEach((item) => {
                  (item.users = true), (item.pid = item.dept || null);
                });
                let A = res.data.concat(ress.data);
                let tmps = GF.utils.buildListToTree(A, 'pid', 'id', 0);
                all[0][this.TreeData.defaultProps.children] = tmps;
                this.treeFilter(all);
              });
            }
            else {
              /*是否显示最末级*/
              let tmps = GF.utils.buildListToTree(
                all[0].children,
                'pid',
                'id',
                0
              ...
              all[0].children = tmps;
              if (!this.TreeData.filter.showchildren) {
                // eslint-disable-next-line no-inner-declarations
                function filers(data, node) {
                  for (let i = 0; i < data.length; i++) {
                    if (
                      data[i].hasOwnProperty('children') &&
                      data[i].children.length > 0
                    ) {
                      filers(data[i].children, data[i]);
                    } else {
                      if (
                        node != undefined &&
    ...
    treeFilter(data) {
        ...
        function each(data, floor) {
          data.forEach((e) => {
            ...
            if (e.hasOwnProperty('children') && e.children.length > 0) {
              ...
              if (floor == _this.TreeData.filter.num + 1) {
                e.children = [];
              }
            }
          });
      ...
      getMaxFloor(data);
      this.Tree.defaultList = data;
      this.Tree.data = data;
      if (this.ones) {
        this.$nextTick(() => {
          this.$refs.tree.setCurrentKey(data[0].id);
          this.ones = false;
          this.$emit('TreeClick', { id: data[0].id, data: data });
        });
      }
    },
          ...
          for (let i = 0; i < Tree.length; i++) {
            for (let x = 0; x < Tree[i].length; x++) {
              ...
              if (Tree[i][x].dicDataType == '2') {
                ...
                //     Tree[i][x].dicData = GF.utils.buildListToTree(
                //       res,
                //       'parentcode',
                //       'code',
                //       0
