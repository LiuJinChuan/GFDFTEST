...
namespace GFDF.Infrastruct.Core.Data
{
    /// <summary>
    /// 数据连接事务的Session接口
    /// </summary>
    ...
    {
        private static readonly Dictionary<string, IDbConnection> ConnDictionary
    = new Dictionary<string, IDbConnection>
    {
        ["sqlconnection"] = new SqlConnection(),
        ["npgsqlconnection"] = new SqlConnection()
    };
        /// <summary>
        /// 根据Provider类型，创建数据库连接
        /// </summary>
        /// <returns></returns>
        private static IDbConnection CreateConnectionByProvider(string connCfg = null)
        {
            var connStringSetting = ConfigurationManager.ConnectionStrings[string.IsNullOrWhiteSpace(connCfg) ? "strConn" : connCfg];

            IDbConnection conn = new SqlConnection(connStringSetting?.ConnectionString ?? connCfg);
        ...
        /// </summary>
        /// <returns></returns>
        public static IDbConnection CreateConnection(string connCfg = null)
        {
            IDbConnection conn = CreateConnectionByProvider(connCfg);
